...
def demo_custom_analog_out_waveform(analogOut, waveform, waveform_duration, wait_duration):
    ...
    status=analogOut.status(channel)

    print(f"analogOut configure result before configuration: {status}")
    conf=analogOut.configure(channel,12)
    status=analogOut.status(channel)
    print(f"Status of the channel{status} ")
    print(f"analogOut configure result after configure : {conf}")
    print()
    ...
    analogOut.nodeFunctionSet(CH1,node,DwfAnalogOutFunction.DC)
    (amplitude_min, amplitude_max) = analogOut.nodeAmplitudeInfo(CH1,node)  # pylint: disable=unused-variable
    '''
